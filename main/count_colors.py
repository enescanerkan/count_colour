"""
Created on August 10, Friday, 2023
...
def update_interface_counts():
    ...
    label_Gray_count.config(text=f"Gri: {colors['Gri']} adet")
...
def update_frames():
    ...
    if is_running:
        ...
        upper_grey = np.array([180, 40, 150])
        grey_mask = cv2.inRange(hsv_frame, lower_grey, upper_grey)
        grey_mask = cv2.erode(grey_mask, kernel, iterations=2)
        grey_mask = cv2.dilate(grey_mask, kernel, iterations=2)
        grey_mask = cv2.GaussianBlur(grey_mask, gaussian_kernel, 0)
        ...
        grey_threshold = area_thresholds['Gri']
        contours, _ = cv2.findContours(grey_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if len(contours) > 0:
            max_contour = max(contours, key=cv2.contourArea)
            if cv2.contourArea(max_contour) > grey_threshold:
                x, y, w, h = cv2.boundingRect(max_contour)
                ...
                if center_x > 300 and not object_in_right['Gri']:
                    object_in_right['Gri'] = True
                    colors['Gri'] += 1
                    cv2.putText(frame, str(colors['Gri']), (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0),
                                2)
                elif center_x <= 300:
                    object_in_right['Gri'] = False
...
label_Gray_count = tk.Label(panel, text="Gri: 0 adet", fg='#808080')
...
label_Gray_count.pack(side=tk.LEFT, padx=20)
